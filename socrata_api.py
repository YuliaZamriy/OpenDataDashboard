...
# https://data.cityofnewyork.us/dataset/Asset-Inventory/r8cp-r4rc/data

def call_socrata_api(uid = 'r8cp-r4rc'):
    """
    Calls Soctata API - username, password and keys can be found in credentials.py

    Kwargs:
        uid: str, Socrate id for the dataset to pull
    Returns:
        pandas df with asset inventory
    """
    ...
    raw_data = request.get(uid, limit=50000)
...
def create_main_dataframe(ai):
    """
    Subsets Asset Inventory to OPPA (official public published assets)
    and selected owners

    Args:
        ai: pandas df, asset inventory

    Returns:
        pandas df with a subset of asset inventory
    """
...
def assign_dataframe_statuses(oppa):
    """
    Assigns status of updates to each data asset

    Args:
        oppa: pandas df with oppa assets
    ...
    status_choices = [pd.Timedelta('7 days'),
                    pd.Timedelta('5 days'),
                    pd.Timedelta('168 days'),
                    pd.Timedelta('25 hours'),
                    pd.Timedelta('47.9 hours'),
                    pd.Timedelta('92 days'),
                    pd.Timedelta('31 days'),
                    pd.Timedelta('1459 days'),
                    pd.Timedelta('84 hours'),
                    pd.Timedelta('16 days'),
                    pd.Timedelta('183 days'),
                    pd.Timedelta('365 days'),
                    pd.Timedelta('50000 days')]
...
def create_asset_status_check(oppa):
    """
    Subsets selected columns from oppa df and renames them

    Args:
        oppa: pandas df

    Returns:
        list of dictionaires-records
        each record -> one dataset
    """

    # create asset status check dataframe and rename columns
    asset_status_check = oppa[['agency',
                                'name',
                                'update_frequency',
                                'automation',
    ...
    asset_status_check = asset_status_check.rename(index=str,
        columns={'agency':'Agency',
                'name':'Asset Name',
                'update_frequency':'Update Frequency',
                'automation':'Update Method',
                'last_update_date_data':'Last Updated',
                'update_status':'Updated on Time?',
                'has_data_dictionary':'Has Data Dictionary?',
                'geocoded':'Is Geocoded?'})

    # convert pandas df to list of ditionaries-records
    return asset_status_check.to_dict('records')

def create_agency_status_check(oppa):
    """
    Aggregates oppa df to agency level and aggregates asset statuses
    ...
    agency_status_check=concat_dfs[['agency',
                                    'total_assets',
                                    'Automated',
                                    'auto_bad_status',
                                    'Manual',
    ...
    agency_status_check=agency_status_check.rename(index=str,
        columns={'agency':'Agency',
                 'total_assets': 'Total Assets',
                 'Automated':'Automated',
                 'auto_bad_status':'Automated Not Updated',
                 'Manual':'Manual',
                 'manu_bad_status':'Manual Not Updated'})

    # convert pandas df to list of ditionaries-records
   return agency_status_check.to_dict('records')
